// Input (JSX):
define(['react', 'Navigation', 'AddPost', 'cookie', 'bootstrap'], function(React, Navigation, AddPost) {
    ...
    var App = React.createClass({displayName: "App",
        ...
        loginUser: function (new_user) {
            console.log("функция loginUser в app.App получен user: " + new_user.username);
        },
        render: function () {
            return (
                React.createElement("div", {className: "app_container"}, 
                    React.createElement(Navigation, {user: this.state.user, l: true}), 
                    React.createElement("div", {className: "container-fluid test-class"}, 
                        React.createElement("div", {className: "row"}, 
                            ...
                            React.createElement("div", {className: "col-xs-12 col-md-9"}, 
                                React.createElement(PostList, {collection: this.state.collection, getNewUser: this.loginUser})
                            )
                        )
                    ), 
